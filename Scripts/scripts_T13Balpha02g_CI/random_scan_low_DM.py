...
flag_scalar_DM = 1
# Flag for subGeV DM (0 = off / 1 = on) this flag is subordinate to the one above
flag_subGeV_DM = 1
...
results_folder = local_path + "Random_scan_subGeVDM_results/"
...
parameter_table_scan = [(-3, 4), (-3, 4), (-6, 0), (-6, 0)]
...
block_entries_scan = [((-3, 3), (-6, 4)), (-6, 0)]
...
###############################
    ...
    if flag_subGeV_DM:
        print("Discarded for not subGeV DM = " + str(discarded_heavy_DM))
    ...
    if flag_scalar_DM:
        ...
        if flag_subGeV_DM:
            if min(scalar_mass_1, scalar_mass_2) > 1.0:
                print("Scalar mass > 1 GeV. Discard point.\n")
                discarded_heavy_DM += 1
                iteration += 1
                continue
    ...
    # Write a SPheno input temporary file
...
#################
    ...
    if SPheno_call_check_1 != -1 or SPheno_call_check_2 != -1 or SPheno_call_check_3 != -1 or SPheno_call_check_4 != -1:
...
####################
    ...
    if flag_relic_density_check:
        ...
        if not (relic_density > relic_density_upper_limit or relic_density < relic_density_lower_limit):
...
print("Some SPheno outputs available in \"" + micrOMEGA_output_spheno_final + "\"")
